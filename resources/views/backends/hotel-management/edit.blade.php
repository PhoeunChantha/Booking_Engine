...
@section('contents')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ __('Edit Hotel Management') }}</h1>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <!-- left column -->
                <div class="col-md-12">
                    <form method="POST" action="{{ route('admin.hotel_management.update', $hotel_management->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="tab-content" id="custom-content-below-tabContent">
                                            @foreach (json_decode($language, true) as $key => $lang)
                                                @if ($lang['status'] == 1)
                                                <?php
                                                            if (count($hotel_management['translations'])) {
                                                                $translate = [];
                                                                foreach ($hotel_management['translations'] as $t) {
                                                    ...
                                                    <div class="tab-pane fade {{ $lang['code'] == $default_lang ? 'show active' : '' }} mt-3"
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="title" id="title_{{ $lang['code'] }}"
                                                                ...
                                                                value="{{ $translate[$lang['code']]['title'] ?? $hotel_management['title'] }}">
                                                            @error('title')
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <textarea id="description_{{ $lang['code'] }}" class="form-control @error('description') is-invalid @enderror"
                                                                name="description[]" placeholder="{{ __('Enter Description') }}" rows="4">{{ $translate[$lang['code']]['description'] ?? $hotel_management['description'] }}</textarea>
                                                            @error('description')
                                                                <span class="invalid-feedback" role="alert">
                                                                    <strong>{{ $message }}</strong>
                                                                </span>
                                            ...
                                            <div class="input-group">
                                                <div class="custom-file">
                                                    ...
                                                    <label class="custom-file-label"
                                                        for="exampleInputFile">{{ $hotel_management->image ?? __('Choose Image') }}</label>
                                                </div>
                                            </div>
