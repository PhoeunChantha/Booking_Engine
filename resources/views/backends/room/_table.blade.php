                ...
                <tr>
                        ...
                        <td>
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input switcher_input status"
                                    id="status_{{ $room->id }}" data-id="{{ $room->id }}"
                                    {{ $room->status == 'active' ? 'checked' : '' }} name="status">
                    ...
                    <td>
                        @if (auth()->user()->can('allotment.view'))
                            <a href="{{ route('admin.room.allotment_calendar.index', ['room_id' => $room->id]) }}"
                                class="btn btn-warning mr-2 text-white btn-edit">
                                <i class="fas fa fa-cog" aria-hidden="true"></i>
                                {{ __('Room allotment') }}
                            </a>
                        @endif
                        @if (auth()->user()->can('rate.view'))
                            <a href="{{ route('admin.rate_plan.index', ['room_id' => $room->id]) }}"
                                class="btn btn-warning mr-2 text-white btn-edit">
                                <i class="fas fa fa-cog" aria-hidden="true"></i>
                                {{ __('Manage Rate Plan') }}
                            </a>
                        ...
                        @if (auth()->user()->can('room.delete'))
                            <form action="{{ route('admin.room.destroy', $room->id) }}"
                                class="d-inline-block form-delete-{{ $room->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" data-id="{{ $room->id }}"
                                    data-href="{{ route('admin.room.destroy', $room->id) }}"
        ...
        <div class="col-12 d-flex flex-row flex-wrap">
            <div class="row" style="width: -webkit-fill-available;">
                <div class="col-12 col-sm-6 text-center text-sm-left pl-3" style="margin-block: 20px">
                    {{ __('Showing') }} {{ $rooms->firstItem() }} {{ __('to') }} {{ $rooms->lastItem() }}
                    {{ __('of') }} {{ $rooms->total() }} {{ __('entries') }}
                </div>
