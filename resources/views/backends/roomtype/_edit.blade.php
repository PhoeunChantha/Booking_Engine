...
<div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('New Room Type') }}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
            </button>
        </div>
        <form action="{{ route('admin.roomtype.update', $roomtype->id) }}" enctype="multipart/form-data" class="submit-form"
            method="post">
                      ...
                      <div class="col-12">
                        ...
                        <div class="tab-content" id="custom-content-below-tabContent">
                            @foreach (json_decode($language, true) as $key => $lang)
                                @if ($lang['status'] == 1)
                                    <?php
                                    if (count($roomtype['translations'])) {
                                        $translate = [];
                                        foreach ($roomtype['translations'] as $t) {
                                    ...
                                    <div class="tab-pane fade {{ $lang['code'] == $default_lang ? 'show active' : '' }} mt-3"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="name" id="name_{{ $lang['code'] }}"
                                                ...
                                                value="{{ $translate[$lang['code']]['name'] ?? $roomtype['name'] }}">
                                            @error('name')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea id="description_{{ $lang['code'] }}" class="form-control @error('description') is-invalid @enderror"
                                                name="description[]" placeholder="{{ __('Enter Description') }}" rows="4">{{ $translate[$lang['code']]['description'] ?? $roomtype['description'] }}</textarea>
                                            @error('description')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
