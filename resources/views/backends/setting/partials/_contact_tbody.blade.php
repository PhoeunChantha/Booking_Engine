        ...
        @foreach (json_decode($contacts, true) as $key => $row)
            <tr>
                ...
                <td>
                    @php
                        $icon = $row['icon'] ?? 'default.png';
                        $iconPath = public_path('uploads/social_media/' . $icon);
                        $fileExists = file_exists($iconPath) && !empty($row['icon']);
                    @endphp
                
                    {{-- <input type="file" class="{{ $fileExists ? 'd-none' : '' }} contact_icon_input_{{ $key }}" name="contact[icon][]"> --}}
                    <input type="file" class="d-none contact_icon_input_{{ $key }}" name="contact[icon][]">
                
                    <img src="{{ asset('uploads/social_media/' . ($fileExists ? $icon : 'default.png')) }}"
                        height="auto" width="60px" style="margin-bottom: 6px; cursor:pointer; border:none !important"
                        alt="" class="avatar border social_media_icon contact_icon_{{ $key }}">
                
                    <input type="hidden" name="contact[old_icon][]" value="{{ $row['icon'] ?? 'default.png' }}">
                </td>                       
                <td>
