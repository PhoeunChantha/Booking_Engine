...
class HighlightContoller extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $hotel_id = $request->current_hotel_id;
            session()->put('current_hotel_id', $hotel_id);
        }

        $current_hotel_id = session()->get('current_hotel_id');
        $highlights = Highlight::where('hotel_id', $current_hotel_id)
            ->latest('id')
            ->paginate(10);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $highlight->hotel_id = session()->get('current_hotel_id');
