...
class RoomController extends Controller
{
    ...
    public function create()
    {
        ...
        $beddings = Bedding::where('status', 1)->get();
        ...
        return view('backends.room.create', compact('language', 'roomtypes', 'beddings', 'default_lang', 'amenities'));
    }
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'max_bedding' => 'nullable|array',
            // 'extra_bedding' => 'nullable|array',
        ]);
        // dd($validator);
        ...
        try {
            ...
            $room->hotel_id        = session()->get('current_hotel_id');
            $room->roomtype_id       = $request->roomtype_id;
            $room->max_room        = $request->max_room;
            $maximumBeddingArray = $request->input('max_bedding'); // This should be an array
            $filteredArray = array_filter($maximumBeddingArray, function ($value) {
                return $value > 0;
            });
            $room->maximum_bedding = json_encode($filteredArray);
            $room->room_size        = $request->room_size;
            $selectAmenities    = $request->input('amenities', []);
            $room->amenities    = $selectAmenities;
            $extraBeddingArray = $request->input('extra_bedding'); // This should be an array
            $filteredArray = array_filter($extraBeddingArray, function ($value) {
                return $value > 0;
            });
            $room->extra_bedding = json_encode($filteredArray);
            ...
            $data = [];
            foreach ($request->lang as $index => $key) {
                if (isset($request->title[$index]) && $request->title[$index] && $key != 'en') {
    ...
    public function edit($id)
    {
        ...
        if ($room->room_size) {
            preg_match('/(\d+)\s*(\D+)/', $room->room_size, $matches);
            $roomSizeValue = $matches[1] ?? '';  // The numeric value
            $roomSizeUnit = $matches[2] ?? '';   // The unit (e.g., m² or ft²)
        } else {
            $roomSizeValue = '';
            $roomSizeUnit = 'm²'; // Default unit
        }
        $amenities = HomeStayAmenity::findorFail(4);
        ...
        return view('backends.room.edit', compact('room', 'roomSizeValue', 'roomSizeUnit', 'beddings', 'roomtypes', 'language', 'default_lang', 'amenities', 'home_stay_gallery'));
    }
